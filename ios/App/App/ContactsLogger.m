...
+ (NSNumber *)nowTimestamp {
    return [NSNumber numberWithUnsignedInteger:[[NSDate date] timeIntervalSince1970] * 1000];
}
...
- (void)saveContactIfNotAlreadySavedWithIdentifier:(NSString *)identifier {
    ...
    [closeContacts enumerateObjectsUsingBlock:^(NSDictionary<NSString *,id> *contact, NSUInteger index, BOOL *stop) {
        ...
        if ([contactUUID isEqualToString:identifier]) {
            ...
            newContact[kContactLastEncounteredKey] = [[self class] nowTimestamp];
            *stop = YES;
        }
    }];
    if (newContact) {
        ...
        NSNumber *nowTimestamp = [[self class] nowTimestamp];
        NSDictionary<NSString *, id> *contact = @{
            kContactDeviceIDKey: identifier,
            kContactFirstEncounteredKey: nowTimestamp,
            kContactLastEncounteredKey: nowTimestamp
        };
